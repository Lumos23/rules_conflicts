...
def alpaca(string_formatter, split='train'):
    ...
    dataset = load_dataset("parquet", data_files = "/scratch/gpfs/lh2046/rule-linter/Adaptive-Finetuning-Attacks-Nov/finetuning_buckets/datasets/data/alpaca/alpaca-train.parquet", split='train')
...
def mixed_pile_retain_magpie_align(string_formatter, split='train'):
    ...
    pile_retain_dataset = load_dataset("parquet", data_files = "finetuning_buckets/datasets/safety_datasets/wmdp/pile_bio.parquet", split='train')
...
def pile_bio(string_formatter, split='train'):
    ...
    """
    dataset = load_dataset("parquet", data_files = "finetuning_buckets/datasets/safety_datasets/wmdp/pile_bio.parquet", split=split)
...
def ctftime(string_formatter, split='train'):
    ...
    """
    dataset = load_dataset("parquet", data_files = "finetuning_buckets/datasets/safety_datasets/ctftime.parquet", split=split)
...
@register_dataset
def wmdp_corpora_bio_forget(string_formatter, split='train'):
    dataset = load_dataset("json", data_files = "/scratch/gpfs/bw1822/cache/bio_remove_dataset.jsonl", split=split)
    def formatter(example):
...
# @register_dataset
# def beavertails(string_formatter, split='train', max_num_samples=-1):
#     """
#     https://huggingface.co/datasets/Beavertail
#     """
#     dataset = load_dataset("json", data_files=f"/home/bw1822/representation-noising/data/beavertails_with_refusals_train.json", split=split)
...
#         formatted_example['messages'].append({'role': 'user', 'content': f"Question: {example['prompt']}\n"})
#         formatted_example['messages'].append({'role': 'assistant', 'content': f"Answer: {example['response']}\n"})
#         return formatted_example
#     dataset = apply_formatter(dataset, formatter)
